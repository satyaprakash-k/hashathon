...
gulp.task('html', ['styles', 'scripts', 'wiredep', 'fileincluder'], function () {
   ...
   * */
  return gulp.src(paths.tmp + '*.html', {cwd: '.'})
  .pipe($.using())
...
//gulp Include Plugin for including html templating
gulp.task('fileincluder', function() {
  return gulp.src([paths.app + '*-template.html'])
  .pipe($.using())
    .pipe($.fileInclude({
      prefix: '@@',
      basepath: '@file'
...
gulp.task('videos', function () {
  return gulp.src(paths.app + 'media/**/*.{mp4,avi}')
  .pipe(gulp.dest(paths.dist + 'media/'))
  .pipe($.size());
});

gulp.task('images', function () {
  return gulp.src(paths.app + 'img/**/*.{png,svg,jpg,jpeg,gif,ico}')
  //.pipe($.cache($.imagemin({
  //  optimizationLevel: 3,
  //  progressive: true,
  //  interlaced: true
...
gulp.task('fonts', function () {
  var fileStream = $.mergeStream(
    ...
    gulp.src(paths.app + 'fonts/*.otf'),
    gulp.src(paths.bower + 'font-awesome-4.4.0/fonts/*'));
...
gulp.task('watch', ['wiredep', 'scripts', 'styles', 'fileincluder'], function () {
  gulp.watch(paths.app + 'styles/**/*.scss', ['styles']);
  gulp.watch('bower.json', ['wiredep']);
  gulp.watch(paths.app + '*.html', ['fileincluder']);
});
